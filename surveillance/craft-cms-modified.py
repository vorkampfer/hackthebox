...
def def_handler(sig, frame):
    print(colored(f"\n\n Exiting craft-cms.py script...", 'red'))
    sys.exit(1)
...
try:
    r = requests.post(url, headers=content_type, data=data, proxies=proxies)
except:
...
r = requests.post(url, data=data, files=files, proxies=proxies) #, proxies={'http' : 'http://127.0.0.1:8080'}) #  
...
r = requests.post(url, data=data, proxies=proxies) #, proxies={"http": "http://127.0.0.1:8080"})
